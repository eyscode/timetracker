...
def login(session, credentials):
    ...
    login_args = {
        ...
        "__VIEWSTATEGENERATOR": login_page.find(
            "input", {"name": "__VIEWSTATEGENERATOR"}
        ).get("value"),
        "__EVENTVALIDATION": login_page.find(
...
def validate_option(form, value, name, _id):
    ...
    options_available = {opt.text: opt.get("value") for opt in options if opt.text}
    if not options_available.get(value):
        names = ", ".join('"{}"'.format(p) for p in options_available.keys())
        raise click.BadParameter(
            '{} "{}" is not available. Choose from: {}'.format(
                name.capitalize(), value, names
            )
        )
...
def fetch_hours(session, form, start, end):
    ...
    args = {
        ...
        "__EVENTVALIDATION": form.find("input", {"name": "__EVENTVALIDATION"}).get(
            "value"
        ),
...
def set_project(session, form, project_option):
    ...
    _eventtarget = re.search(
        r"hiddenField\|__EVENTTARGET\|([\w*/*\+*=*]*)", str(content)
    ).groups()[0]
    _eventargument = re.search(
        r"hiddenField\|__EVENTARGUMENT\|([\w*/*\+*=*]*)", str(content)
    ).groups()[0]
    _lastfocus = re.search(
        r"hiddenField\|__LASTFOCUS\|([\w*/*\+*]*=*)", str(content)
    ).groups()[0]
    _viewstate = re.search(
        r"hiddenField\|__VIEWSTATE\|([\w*/*\+*]*=*)", str(content)
    ).groups()[0]
    _viewstategenerator = re.search(
        r"hiddenField\|__VIEWSTATEGENERATOR\|([\w*/*\+*=*]*)", str(content)
    ).groups()[0]
    _eventvalidation = re.search(
        r"hiddenField\|__EVENTVALIDATION\|([\w*/*\+*]*=*)", str(content)
    ).groups()[0]
    secrets = {
...
def hours_as_table(content, current_month, full, show_weekday):
    ...
    if full:
        ...
        values[column_headers.index("Hours")] = (
            rows[-1].find_all("td")[1].string
    ...
    load_time_args.update(
        {
            ...
            "ctl00$ContentPlaceHolder$idTipoAsignacionDropDownList": options[
                "assignment"
            ],
            "ctl00$ContentPlaceHolder$idFocalPointClientDropDownList": options.get(
                "focal"
            ),
...
def load_csv_hours(csvfile, config):
    """
    Load csv file hours
    """
...
def load_hours(text, config, date, pto, vacations, hours):
    ...
    if hours is None:
        raise click.BadParameter(
            "You need to specify hours amount with --hours (-h) or using hours options in config.toml"
        )
    ...
    project_option = validate_option(
        load_time_page, options.get("project"), "Project", PROJECT_DROPDOWN
    )
    ...
    if not pto and not vacations:
        focal_option = validate_option(
            load_assigments_page, options.get("focal"), "Focal", FOCAL_DROPDOWN
        )
...
def show_hours(config, start, end, full, weekday):
    ...
    click.echo(
        hours_as_table(
            hour_list,
            current_month=not start and not end,
            full=full,
